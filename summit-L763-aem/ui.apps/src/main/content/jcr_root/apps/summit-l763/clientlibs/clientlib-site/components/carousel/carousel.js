 ...
 function applyComponentStyles() {
  ...
  $(".cmp-carousel-slider").not("[data-carousel-processed='true']").each(function() {
            ...
            $.getJSON(jsonURI, function(result){
                $.each(result[":items"].root[":items"], function(cfindex, cfragment) {

                 if (cfragment[":type"] === "summit-l763/components/content/contentfragment") {
                    $(section).append("<div><div class='speaker-img'><img src='" + cfragment.elements.speakerImage.value + THUMBNAIL_RENDITION + "' /><div class='border-box-highlight'></div></div><h4>" + cfragment.elements.speakerName.value + "</h4></div>");
                    }
                });
            }).done(function() {
            $(section).slick({
